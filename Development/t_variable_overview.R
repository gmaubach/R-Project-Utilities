...
t_module_name = "t_variable_overview.R"
...
t_variable_overview <- function(dataset) {
    #' @title Generate a overview of all variables in the dataset.
    #'
    #' @description The function generates a table containing a
    ...
    #'
    #' @rational
    #' The overview of all variables shall help to determine which of the
    ...
    for (variable in variables) {
        ...
        if (inherits(variable, "character")) {
            ...
            overview["Min"]      <- NA
            overview["Rng"]      <- NA
            overview["Distinct"] <- f_distinct(variable)
            overview["hasNA"]    <- sum(is.na(variable))
            overview["hasNaN"]   <- NA
            overview["hasInf"]   <- NA
            overview["Conf"]     <- NA
            overview["Req"]      <- NA
            ...
            overview["Lin"]      <- NA
            overview["Vartype"]  <- "T"
            overview["Class"]    <- class(variable)[1]
            ...
            overview["Min"]      <- NA
            overview["Rng"]      <- NA
            overview["Distinct"] <- NA
            overview["hasNA"]    <- sum(is.na(variable))
            overview["hasNaN"]   <- sum(is.nan(variable))
            overview["hasInf"]   <- sum(is.infinite(variable))
            overview["Conf"]     <- NA
            overview["Req"]      <- NA
            overview["Var"]      <- NA
            overview["Lin"]      <- NA
            ...
            overview["Max"]      <- max(variable, na.rm = TRUE)
            overview["Min"]      <- min(variable, na.rm = TRUE)
            overview["Rng"]      <- diff(overview["MIN"], overview["MAX"])
            overview["Distinct"] <- f_distinct(variable)
            overview["hasNA"]    <- sum(is.na(variable))
            overview["hasNaN"]   <- sum(is.nan(variable))
            overview["hasInf"]   <- sum(is.infinite(variable))
            overview["Conf"]     <- NA
            overview["Req"]      <- NA
            overview["Var"]      <- f_desc_variance(variable)
            overview["Lin"]      <- NA
            overview["Vartype"]  <- "N"
        }
    }
