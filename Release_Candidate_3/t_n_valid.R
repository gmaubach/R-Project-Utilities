...
t_version = "2016-06-16"
t_module_name = "t_n_valid.R"
t_status = "released"
...
# [ Function Defintion ]--------------------------------------------------------
t_n_valid <- function(dataset,
                     variables = "all") {
  # Counts the number of valid values across variables within given set of
  # veriables
  #
  # Args:
  ...
  #
  # Returns:
  #   Original dataset with variable "n_valid" added.
  #
  # Error handling:
  #   None.
  #
  ...
  dataset[["n_valid"]] <- apply(dataset[,variable_list],
                                1, 
...
t_test <- function(do_test = FALSE) {
  if (do_test == TRUE) {
    ...
    cat("\n", "\n", "Function call", "\n", "\n")
    ds_example <- t_n_valid(dataset = ds_example,
