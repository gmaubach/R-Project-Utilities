...
# License       : GPL-2
#
# Copyright (C) by Wein Wolf Import GmbH & Co. Verwaltungs KG 2016
#
...
t_find_originals_and_duplicates <- function(dataset,
  ...
  #   key_variable (string):
  #     Variable name as string that shall be used to identify duplicates.
  ...
  dataset["originals_and_duplicates"] <- FALSE

  for (item in unique_duplicates) {
    originals_and_duplicates_per_item <- dataset[key_variable] == item
    dataset["originals_and_duplicates"] <-
      dataset["originals_and_duplicates"] | originals_and_duplicates_per_item
  }
